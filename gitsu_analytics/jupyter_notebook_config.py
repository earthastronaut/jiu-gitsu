...
print('blah')
#------------------------------------------------------------------------------
# Copied these lines from the default jupyter_notebook_config.py
# set up in the jupyter docker stacks
...
def post_save_hook(model, os_path, contents_manager):
    ...
    cwd, fname = os.path.split(os_path)
    save_progress_indicator = os.path.join(cwd, SAVE_PROGRESS_INDICATOR_FILE_PY)

    if os.path.exists(save_progress_indicator):
        logging.info('Post save script conversion ' + os_path.replace('.ipynb', '.py'))
        check_call(['jupyter', 'nbconvert', '--to', 'script', fname], cwd=cwd)
    else:
        logging.info('No post save indicator ' + save_progress_indicator)
